...
export default function Customers() {
  ...
  const handleSelectedProductChange = async (event: SelectChangeEvent) => {
    ...
    await TransactionsService.transactions(event.target.value)
      .then((response) => {
  ...
  return (
    <div className="flex-1 items-center justify-center p-7">
      ...
      <div className="w-4/5 py-6 md:w-2/5">
            ...
            <TableBody>
              {transactions.map((item) => (
                <StyledTableRow key={item.transaction.id}>
                  ...
                  <StyledTableCell align="center">
                    {item.transaction.customer.category}
                  </StyledTableCell>
                  <StyledTableCell align="center">
                    {item.transaction.customer.identity}
                  </StyledTableCell>
                  <StyledTableCell align="center">
                    R$ {item.product.price - item.transaction.discount},00
                  </StyledTableCell>
                  <StyledTableCell align="center">
                    {item.transaction.paymentMethod === '2' &&
                      'Catão de Crédito'}
                    {item.transaction.paymentMethod === '6' && 'Pix'}
                  </StyledTableCell>
                  <StyledTableCell align="center">
                    {item.transaction.installments}
                  </StyledTableCell>
                  <StyledTableCell>{item.product.description}</StyledTableCell>
                  <StyledTableCell align="center">
                    {transformDate(item.transaction.createdAt)}
                  </StyledTableCell>
